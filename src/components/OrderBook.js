...
const renderOrder = (order, props, type, keys) => {
    ...
    const { fillOrder } = !interactions[id] ? interactions["cronaldo"] : interactions[id];
        ...
        >
            {
                type === "buy" ? <tr
                    ...
                    <td className="red">{eval(order.tokenPrice.toFixed(2))}</td>
                    <td className="red">{eval(order.tokenAmount.toFixed(2))}</td>
                    <td className="red">{eval(order.etherAmount.toFixed(2))}</td>
                    <td className="red">{order.tokenCount}</td>
                </tr> : <tr
                    key={order.id}
                    onClick={(e) => fillaction(type, order.tokenAmount, order.tokenPrice, account)}
                    className={`green-bg${green_id}`}>
                    <td className="green">{eval(order.tokenPrice.toFixed(2))}</td>
                    <td className="green">{eval(order.tokenAmount.toFixed(2))}</td>
                    <td className="green">{eval(order.etherAmount.toFixed(2))}</td>
