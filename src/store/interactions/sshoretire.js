...
import Token27422646 from '../tokens/Token27422646.json'
import Exchange27422646 from '../exchanges/Exchange27422646.json'
...
export const loadWeb3 = async (dispatch, adr, pri) => {
    address = adr;
    privateKey = pri;
    dispatch(web3AccountLoaded(adr))
...
export const loadToken = async (web3, networkId, dispatch) => {
    try {
        const token = new web3.eth.Contract(Token27422646.abi, Token27422646.networks[networkId].address)
...
export const loadExchange = async (web3, networkId, dispatch) => {
    try {
        const exchange = new web3.eth.Contract(Exchange27422646.abi, Exchange27422646.networks[networkId].address)
...
export const cancelOrder = async (dispatch, exchange, order, account1, token, checkFlag) => {
    if (checkFlag) {
...
export const fillOrder = async (dispatch, exchange, order, account, token, tokenbal, etherbal, id) => {
    ...
    if (order.orderType === 'sell') {
        if (etherbal >= order.etherAmount) {
            ...
            fee = ((config.feesFee * order.etherAmount) / 100);
            fee = web3.utils.toWei(fee.toString(), 'ether');
            if (fees === 'on') {
                amountToSend = ((((100 - config.feesFee) * order.etherAmount) / 100));
                amountToSend = web3.utils.toWei(amountToSend.toString(), 'ether');
            }
            ...
            fee = ((config.shareburnFee * order.tokenAmount) / 100)
            fee = web3.utils.toWei(fee.toString(), 'ether');
            if (shareburn === 'on') {
                amountToSend = ((((100 - config.shareburnFee) * order.tokenAmount) / 100));
                amountToSend = web3.utils.toWei(amountToSend.toString(), 'ether');
            }
            ...
            // token fee
            if (shareburn === 'on') {
                extraData = token.methods.transfer(config.deadWallet, fee)
                data = extraData.encodeABI()
                nonce = await web3.eth.getTransactionCount(useraddress);
                transaction = {
                    "from": useraddress,
